            ...
            if not (args.skip_trimming or args.skip_mapping or args.skip_calling): # input= LIB.FILE1 (fastq)
                ...
                if args.skip_calling: # skip_calling, input = methylCALL
                    ...
                    if args.__dict__['calling_data'] != None:
                        ...
                        if os.path.exists(callD):
                            args.__dict__['calling_data'] = callD
                        else:
                            check_line += f'\n*** calling data ({callD}) is not existed (skip_calling)'
                            exit = True
                    ...
                    if not 'bam_file' in lib.param_keys():    # When there's no input bam file, search for pipeline output
                        lib.bam_file = f'{os.path.abspath(args.out)}/methylCALL/{lib.lib_name}/data/{lib.lib_name}_bismark_mapping.bam'
                ...
                elif args.skip_trimming: # skip_trimming, input = LIB.TRIMMED_FILE1,2:
                    if not 'trimmed_file_1' in lib.param_keys():    # When there's no input trimming file, search for pipeline output
                        if lib.lib_type == 'P':
                            lib.trimmed_file_1 = f'{os.path.abspath(args.out)}/methylCALL/{lib.lib_name}/data/*_val_1.fq.gz'
                            lib.trimmed_file_2 = f'{os.path.abspath(args.out)}/methylCALL/{lib.lib_name}/data/*_val_2.fq.gz'
                        else:
                            lib.trimmed_file_1 = f'{os.path.abspath(args.out)}/methylCALL/{lib.lib_name}/data/*_trimmed.fq.gz'
...
def Bismark_mapping(callD, refD):
    ...
    for i in range(len(LIBs)):
        ...
        cmd_bismark_align = f'{prog.bismark} --score_min L,0,-0.6 -N 0 -L 20 --parallel {core} --temp_dir {tmpD} {refD} -o {dataD}'
        if LIB.lib_type == 'P':
            cmd_bismark_align += f' -1 {LIB.trimmed_file_1} -2 {LIB.trimmed_file_2}'
        
        elif LIB.lib_type == 'S':
            cmd_bismark_align += f' {LIB.trimmed_file_1}'
        
        CMD_list.append( [cmd_bismark_align, f'{libD}/logs/log.Bismark.txt',log_proc ] )
...
def VISandDMR(outD):
    ...
    wig_cmd =  f'Rscript {binD}/vis_script/visualization_parallel.R {vis_param_file} {resultD}/Ideogram.bed {resultD}/ {args.core}'
        ...
        for AN in DMR.param_values():
            ...
            ##get methylation Levels on promoter region
                ...
                if not os.path.isfile(methylLevelF):
                    ...
                    ## getDMR
                    completed = subproc(f'{prog.dmr} {rawF} {args.qvalue} 1> {analD}/DMR.bed', f'{anal_logD}/log.getDMR.txt',log_proc)
                    return completed
...
if __name__ == "__main__":
    ...
    completed = Bismark_calling(callD)
    if not completed:
